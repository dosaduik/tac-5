...
def get_claude_env() -> Dict[str, str]:
    ...
    # Inherit the parent environment and overlay the ADW-specific values.
    # Windows needs SystemRoot/SystemDrive/TEMP/APPDATA etc. present or the
    # Claude Code (Bun) binary fails with:
    #   "The %SystemRoot% environment variable is not set. Bun needs this set
    #    in order for network requests to work."
    env = os.environ.copy()
    env.update({k: v for k, v in required_env_vars.items() if v is not None})
    return env
