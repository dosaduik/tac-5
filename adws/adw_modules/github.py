...
def get_github_env() -> Optional[dict]:
    ...
    A hand-built minimal env is not safe here: gh needs far more than PATH
    (SystemRoot, APPDATA, USERPROFILE, TEMP, proxy settings), so this copies
    the real environment and only adds GH_TOKEN.
    """
    github_pat = os.getenv("GITHUB_PAT")
    if not github_pat:
        return None

    # Inherit the full parent environment, then overlay the token.
    # A minimal env breaks gh on Windows: the TLS/DNS stack needs SystemRoot,
    # and gh needs APPDATA/USERPROFILE/TEMP plus any proxy vars. Without them
    # every call fails with "error connecting to api.github.com".
    env = os.environ.copy()
